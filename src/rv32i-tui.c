...
rv32i_debugger_tui_s rv32i_init_ncurses(void)
{
	...
	tui.register_dump_win = newwin(RV32I_REGISTERDISPLAY_HEIGHT, RV32I_REGISTERDISPLAY_WIDTH, 2, 0);
	tui.disassemble_win   = newwin(RV32I_DISASSEMBLY_HEIGHT,     RV32I_DISASSEMBLY_WIDTH,    12, 0);
	tui.memory_dump_win   = newwin(RV32I_MEMORYDUMP_HEIGHT,      RV32I_MEMORYDUMP_WIDTH,      2,64);
...
void rv32i_debugger_tui_draw_registers(rv32i_hart_s* cpu)
{
	...
	mvwchgat(rw, 6, 11, -1, A_BOLD, COLOR_CYAN,   NULL);
	mvwchgat(rw, 7, 11, -1, A_BOLD, COLOR_CYAN,   NULL);

	mvwchgat(rw, 0, 11, -1, A_BOLD, COLOR_YELLOW, NULL);
	mvwchgat(rw, 7, 49, -1, A_BOLD, COLOR_YELLOW, NULL);

	mvwchgat(rw, 3, 11, -1, A_BOLD, COLOR_RED,    NULL);

	mvwchgat(rw, 3, 23, -1, A_BOLD, COLOR_BLUE,   NULL);
	mvwchgat(rw, 4, 11, -1, A_BOLD, COLOR_BLUE,   NULL);
	mvwchgat(rw, 5, 11, -1, A_BOLD, COLOR_BLUE,   NULL);
...
void rv32i_debugger_tui_draw_disassembly(rv32i_hart_s* cpu)
{
	...
	int j = 0; for (unsigned int i = cpu->pc-32; i != cpu->pc+36; i+=4)
	{
		...
		/* "" cpu->pc */
		mvwprintw(dw, j++, 50, "%s", cur ? "<-" : "");

		/* Colors */
		if (null)    mvwchgat(dw, j - 1, 0, -1, A_DIM,  COLOR_WHITE,  NULL);
		if (unknown) mvwchgat(dw, j - 1, 0, -1, A_BOLD, COLOR_YELLOW, NULL);
		if (cur)     mvwchgat(dw, j - 1, 0, -1, A_BOLD, COLOR_GREEN,  NULL);
...
void rv32i_debugger_tui_draw_memorydump(rv32i_hart_s* cpu)
{
	...
	mvwin(mw, 2, cpu->tui.max_width - RV32I_MEMORYDUMP_WIDTH);

	mvwprintw(mw, 0, 0, "Frame: At %08x", cpu->regs[8]);
	...
	for (unsigned int i = cpu->regs[8]-96; i != (unsigned) cpu->regs[8]+96; i++)
	{
		...
		{
			mvwprintw(mw, 2 + j, (k*3), ".. ");
			mvwchgat (mw, 2 + j, (k*3), 3, A_DIM, COLOR_WHITE, NULL);
			datastring[k] = '.';
			goto skip;
		};
		...
		mvwprintw(mw, 2 + j, (k*3), "%02x", value);
		...
		if (k == 8)
		{
			mvwprintw(mw, 2 + j, (k*3), "  |%s|", datastring);
			k = 0, j++;
		}
